...
json_value json_read(const char * const path) {
    ...
    if (!fp) {
        ...
        return undefined_json; // 정의된 undefined_json 값을 반환
    }
    ...
    char *json_str = (char *)malloc(filesize + 1);
    if (!json_str) {
    ...
    size_t read_size = fread(json_str, 1, filesize, fp);
    json_str[read_size] = '\0';

    fclose(fp);

    json_value root = json_create(json_str);
    free(json_str);
